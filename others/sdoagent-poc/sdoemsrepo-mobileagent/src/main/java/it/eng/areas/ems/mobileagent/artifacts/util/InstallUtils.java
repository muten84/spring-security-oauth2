...
public class InstallUtils {
	...
	public static boolean emptyDestination(String targetPath) throws IOException {
	...
	public static boolean installationDone(String zipFilePath) {
		...
		if (f.exists()) {
			...
			try {
				...
				Logger.info("IOException in installationDone " + e.getMessage());
				return false;
			}
		}
		return false;
	...
	public static boolean clearAppCache(String app) {
		...
		try {
			...
			Logger.info("IOException in clearAppCache: " + e.getMessage());
			return false;
		}
		return true;
	}
	...
	public static void main(String[] args) {
		...
		System.setProperty("agent.server.rootFolder", "/esel/terminal");
		System.out.println("delta folder prepared: " + InstallUtils.prepareWebAppForDelta());
	}

	/**
	 * @param localInstallPath
	 */
	public static boolean prepareWebAppForDelta() {

		String wwwFolder = System.getProperty("agent.server.rootFolder") + "/app";
		Collection<File> files = FileUtils.listFiles(new File(wwwFolder),
				new String[] { "chunk.js", "bundle.js", "bundle.css","html" }, false);
		boolean deleted = true;
		for (File file : files) {
			System.out.println("file to remove for delta: " + file.getName());
			deleted &= file.delete();
		}
		return deleted;
