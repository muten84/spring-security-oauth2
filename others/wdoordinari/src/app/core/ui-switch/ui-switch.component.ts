...
@Component( {
  ...
  [style.background-color]="getColor(null)"
...
export class UiSwitchComponent implements ControlValueAccessor {
    ...
    @Input() color = 'rgb(0, 128, 157)';
    @Input() switchOffColor = '';
    @Input() switchColor = '#fff';
    defaultBgColor = '#c0c0c0';
    defaultBoColor = '#4d4d4d';
    ...
    onToggle() {
        ...
        if ( this.value ) {
            this.onChangeCallback( this.checked ? this.value : undefined );
            this.onTouchedCallback( this.checked ? this.value : undefined );
        } else {
