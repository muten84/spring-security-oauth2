export function tokenNotExpired(tokenName, jwt?: string): boolean {
...
export function getUserName(tokenName, jwt?: string): string {

    const token: string = jwt || sessionStorage.getItem(tokenName);

    const jwtHelper = new JwtHelper();

    return token != null? jwtHelper.getUserName(token):null;
}
