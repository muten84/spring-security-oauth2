...
import { BookingModuleServiceService, AAAControllerService } from '../../services/services.module';
...
export class RootComponent implements OnInit, AfterViewInit {
    ...
    constructor(
        ...
        private userService: AAAControllerService,
        ...
        this.items = [
            ...
            { 
                sectionId: 'menu3', name: 'Turni', path: '/sinottico-turni', icon: '', iClass: 'fa fa-calendar', childItems: [
                    { name: 'Nuovo Turno', path: '/gestione-turno' }
                ] 
            }, 
    ...
    public onRightItemClick(itemName: string) {
        ...
        switch (itemName) {
            case 'synoptic-prenotazioni': {
                ...
                /*this.router.navigate(['/sinottico-prenotazioni']));*/ 
                this.showConfirmDialog("Attenzione!", "Andranno perse eventuali modifiche effettuate.\nSi desidera continuare con l'operazione?").then((result) => {
    ...
    public showToastMessage(type, message) {
        switch (type) {
            case 'error': {
                this.toastr.error(message, 'Errore', { enableHtml: true, closeButton: true, positionClass: 'toast-bottom-right' });
                return;
            }
            case 'warning': {
                this.toastr.warning(message, 'Attenzione', { enableHtml: true, closeButton: true, positionClass: 'toast-bottom-right' });
                return;
            }
            case 'success': {
                this.toastr.success(message, '', { enableHtml: true, positionClass: 'toast-bottom-right' });
                return;
            }
            default:
